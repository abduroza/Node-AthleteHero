...
async function getScholarship(req, res){
    try {
        let scholarship = await Scholarship.findById(req.params.id)
            .select('_id title quota description image total_fund deadline start_date end_date id_sport_category id_locations id_investor id_user list_id_applied date_create')
            .populate({path: ''})
...
async function getAllScholarship(req, res){
    let scholarship = await Scholarship.find({})
        .select('_id title quota description image total_fund deadline start_date end_date id_sport_category id_locations id_investor id_user date_create')
        .populate({path: 'id_investor', select: ['_id', 'name', 'phone', 'address']})
        .populate({path: 'id_user', select: ['_id','email','fullname','image','role']})
...
module.exports = {addScholarship, editScholarship, getScholarship, getAllScholarship, getScholarshipUserInvestor, deleteScholarship}
